            ...
            @if ($stage->state === 'step' || $stage->state === 'action')
                <div class="flex flex-col rounded-full items-center text-center space-y-6">
                        @if ($getRecord()->stage->order > $stage->order)
                            <div class="rounded-full p-4 shadow-sm" style="
                            @if ($getRecord()->stage->state == "aborted")
                                background-color: rgba(var(--danger-50), 1); border-color: rgba(var(--danger-400), 0.1); border-width: 2px;
                            @else
                                background-color: rgba(var(--success-50), 1); border-color: rgba(var(--success-400), 0.1); border-width: 2px;
                            @endif
                            ">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" class="w-6 h-6" style="
                                @if ($getRecord()->stage->state == "aborted")
                                    stroke: rgba(var(--danger-600), 1)
                                @else
                                stroke: rgba(var(--success-600), 1)
                                @endif
                                " fill="none">
                                    @if($getRecord()->stage->state == "aborted")
                            ...
                            <p class="text-sm font-normal" style="
                            @if ($getRecord()->stage->state == "aborted")
                        ...
                        @elseif ($getRecord()->stage->order === $stage->order)
                            ...
                            @if ($getRecord()->stage->state == "aborted")
                                background-color: rgba(var(--danger-50), 1); border-color: rgba(var(--danger-400), 0.1); border-width: 2px;
                            @else
                                background-color: rgba(var(--info-50), 1); border-color: rgba(var(--info-400), 0.1); border-width: 2px;
                            @endif
                            ">
                            ...
                            <p class="text-sm font-medium" style="
                            @if ($getRecord()->stage->state == "aborted")
                                color: rgba(var(--danger-600), 1);
                            @else
                                color: rgba(var(--info-600), 1);
                            @endif
                            ">{{ $stage->name }}</p>
                        @else
                    ...
                    @if ($getRecord()->stage->order - 1 >= $stage->order)
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-4 h-4 -mt-6" style="
                            @if ($getRecord()->stage->state == "aborted")
                                stroke: rgba(var(--danger-600), 1)
                            @else
                            stroke: rgba(var(--success-600), 1)
            ...
            @elseif($stage->state === 'completed')
                <div class="flex flex-col items-center text-center space-y-6">
                    @if($getRecord()->stage->order >= $stage->order)
                        ...
                        @endif
                        ">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" class="w-6 h-6" fill="none" style="
                                @if ($getRecord()->stage->state == "aborted")
                                    stroke: rgba(var(--danger-600), 1)
                                @else
                                stroke: rgba(var(--success-600), 1)
